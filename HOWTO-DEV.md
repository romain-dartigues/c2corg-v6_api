...
- Python 3.9.x installed with pip and the virtualenv package (currently, it will not work with python 3.10+)
    ...
    - The recommended, and arguably the easiest way, is to use [uv](https://docs.astral.sh/uv/getting-started/installation/)
- An OCI compatible environment with docker-compose plugin (Docker, [Docker desktop](https://www.docker.com/products/docker-desktop/), [podman](https://podman.io/), [podman desktop](https://podman-desktop.io/) or a manual installation)
...
- Activate the virtual environment (⚠️ Do not forget to activate it in all terminal instances you intend to use)
    ```shell
    # when you use uv, just prefix your commands with it, such as: uv run python... uv run pytest...
    # otherwise:
    # Linux (or WSL)
...
- Install the API and its python dependencies in the virtual environment with the following command:
    ```shell
    # once again, if you use uv it will be automatically done, but you can:
    uv pip install -e .[dev]
    # otherwise:
    pip install -e ".[dev]"
    ```
- Start the necessary tools (postgres database, redis, and elasticsearch) in OCI containers:
    ```shell
    docker compose up -d
    ```
